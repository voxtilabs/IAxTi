...
| Transferencia internacional · GLM / Zhipu (IA) | A.5.19 | 7.5 | 8.2 | — | transferencia internacional | ADR-0023 + `proveedorPermitidoParaTarea` y `vaRedactadoAlProveedor` | T/L/C | parcial — **decidido y aplicado en el código** (ADR-0023, 21/09/2026): `glm` solo acepta `clasificar`, y ese prompt sale REDACTADO; cualquier otra tarea configurada con `glm` cae al proveedor por defecto, y `glm` no puede ser el modelo económico. Lo que falta es contractual, y son tres papeles para nuestra cuenta: que no entrenan con lo que mandamos, la "no retención" activada con su plazo en días, y qué entidad contrata y bajo qué jurisdicción. Revisados sus términos el 21/09/2026: no publican lo primero ni lo tercero; lo segundo existe desde el 20/09/2026 pero es opt-in a pedido. El destino sigue siendo China, con un marco legal distinto al de Google o Anthropic |
