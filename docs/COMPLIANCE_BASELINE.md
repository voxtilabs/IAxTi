...
| Vigilancia de disponibilidad y alertas | A.8.16 | — | — | — | disponibilidad | #17 | T/O | **parcial** — `/health` y `/ready` en los cinco servicios; Compose comprueba vida también en workers y agents. Web/admin verifican API y Supabase Auth con plazo y errores redactados. Pendientes: endurecer la sonda de consumidores, recepción real de telemetría en los destinos, Uptime Kuma y las cuatro alertas mínimas (error rate, p95, cola atascada, disco). Procedimiento en `docs/runbooks/OBSERVABILIDAD.md` |
