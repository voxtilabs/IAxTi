...
export async function retryOutboundDelivery(
  ...
  if (!m.rowCount) throw new Error('No encontramos ese mensaje en esta conversación.');
  if (m.rows[0].delivery_status !== 'queued') throw new Error('Solo se recuperan mensajes pendientes de despacho.');
  ...
  if (eventId === null) throw new Error('Este mensaje no tiene un pedido de despacho agotado.');
