...
const CACHE = new Map<string, { plan: string; modulos: string[] | null; at: number }>();
...
/**
 * `modulos: null` significa "no sé": tenant desconocido, id que no es uuid o
 * la base que no contesta. Este portón decide cobro, no seguridad — el
 * permiso ya se verificó antes —, así que ante la duda NO se corta.
 */
export async function modulosDelPlan(
  client: PoolClient | Pick<Pool, 'query'>,
  tenantId: string,
): Promise<{ plan: string; modulos: string[] | null }> {
  ...
  const r = await client
    ...
    .catch(() => ({ rows: [] as Array<{ plan: string; modules: string[] }> }));
  const plan = (r.rows[0]?.plan as string) ?? '';
  const modulos = r.rows.length > 0 ? ((r.rows[0].modules as string[]) ?? []) : null;
...
export function accesoAlModulo(input: {
  ...
  if (input.delPlan === null) return 'completo'; // no sé qué plan tiene: no corto
